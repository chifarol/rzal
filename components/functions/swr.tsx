...
export const modelsData = () => {
  const { data, error, isLoading } = useSWR(
    "/wp-json/wp/v2/vehicle_models?_fields[]=id&_fields[]=title&_fields[]=slug&_fields[]=acf&_fields[]=yoast_head&_fields[]=yoast_head_json",
    (url) =>
      axios
        .get(process.env.NEXT_PUBLIC_BACKEND_SERVER_URL + url)
        .then((res) => res.data),
...
export const manufacturersData = () => {
  const { data, error, isLoading } = useSWR(
    "/wp-json/wp/v2/vehicle_manufacturers?_fields[]=id&_fields[]=title&_fields[]=slug&_fields[]=acf&_fields[]=yoast_head&_fields[]=yoast_head_json",
    (url) =>
      axios
        .get(process.env.NEXT_PUBLIC_BACKEND_SERVER_URL + url)
        .then((res) => res.data)
